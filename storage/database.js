...
let data = {
    ...
    flashcards: [],
    goal: { daily: 60, progress: 0, date: new Date().toISOString().split('T')[0] }
};
...
export async function initDB() {
    ...
    if (!data.goal) {
        data.goal = { daily: 60, progress: 0, date: new Date().toISOString().split('T')[0] };
    }
    // Reset progress if new day
    const today = new Date().toISOString().split('T')[0];
    if (data.goal.date !== today) {
        data.goal.progress = 0;
        data.goal.date = today;
        save('GOAL');
    }
    await openDB();
}
...
export function updateChapter(id, updates) {
    ...
    if (idx !== -1) {
        ...
        if (updates.progress !== undefined) {
            ...
            updateStreak();
            // Add to goal progress (e.g., each progress update counts as 5 minutes)
...
export function setGoal(minutes) {
    data.goal.daily = minutes;
    save('GOAL');
}
export function updateGoalProgress(minutes) {
    data.goal.progress += minutes;
    if (data.goal.progress > data.goal.daily) data.goal.progress = data.goal.daily;
    save('GOAL');
}
